...
format:
  revealjs: 
    ...
    footer: "ivelasq-automating-quarto.share.connect.posit.cloud"
---
...
-   Slides: <https://ivelasq-automating-quarto.share.connect.posit.cloud>
...
::: notes
We choose Quarto because it streamlines our entire workflow and ensures we're doing modern, reliable data science. The biggest advantages are Reproducibility and Automation: Quarto integrates your code and narrative, guaranteeing that anyone can reproduce your results by simply rerunning the document, and it automatically updates your reports when the underlying data changes, saving you huge amounts of time. Plus, it offers excellent Flexibility—you can combine code from multiple languages and add complex figures and tables easily. Finally, Quarto provides Single-Source Publishing, which means you write your report once and can instantly generate professional HTML webpages, PDFs, and Word documents from that one file, eliminating the need for constant re-formatting. Shirley Han has a great YouTube video from the R NYC Conference on Quarto reports. She mentions that if you are writing a report repeatedly, so not just one time, where the structure stays the same but the data or numbers or figures change, then Quarto may be for you. Also, if you are already using a Quarto-supported language for data cleaning, creating graphs, etc., then it might be a good fit. 
:::
...
````{.markdown filename="report.qmd" code-line-numbers=|8-21}
---
title: "Polling Places Report - Alabama"
format: html
---
...
![](images/code-render.png){fig-align="center" style="box-shadow: 5px 5px 15px rgba(0, 0, 0, 0.3); border-radius: 5px;"}
...
![](images/code-options-render.png){fig-align="center" style="box-shadow: 5px 5px 15px rgba(0, 0, 0, 0.3); border-radius: 5px;"}
...
The number of counties is `r counties`.
````
...
````{.markdown filename="_template.qmd" code-line-numbers=|1,3,9}
### BALDWIN COUNTY
...
::: notes
Now, anytime a county is mentioned, we use a placeholder instead.
:::
...
::: notes
This code block is designed to automate the creation of customized documents for every single county in Alabama. It starts by doing the necessary prep work: filtering the polling_places data to include only rows where the state is "Alabama," and then extracting a simple checklist of all the unique county names. Next, the code enters the automation phase by using map (a looping function) to iterate over every county on that list. For each county, the knitr::knit_expand function acts like a smart mail merge, taking a generic template file (\_template.qmd) and filling in the placeholders with the specific, current county name. This creates 67 individual, customized blocks of content. Finally, the code uses knitr::knit_child to take all those separate, expanded pieces of text, unlist them, and stitch them together sequentially into one single, cohesive document, ready to be included in your main report. In short, it turns a list of counties into a single, comprehensive report with a dedicated section for each one.
:::
...
````{.markdown filename="_parameters_render.R" code-line-numbers=|1-3|5-6|8-10|11|12-19|20}
library(readr)
library(dplyr)
library(quarto)
...
* You can avoid a lot of headaches with Quarto
